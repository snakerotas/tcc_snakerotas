...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $confirm  = isset($_POST['confirm-password']) ? $_POST['confirm-password'] :'';
    ...
    if (empty($errors)) {
        ...
        $stmt = $pdo->prepare("SELECT id FROM usuarios WHERE usuario = :u OR email = :e LIMIT 1");
        ...
        if ($stmt->fetch()) {
            ...
            $ins->execute([':u'=>$username, ':n'=>$nome, ':e'=>$email, ':s'=>$hash]);
...
.signup-container {
  ...
  text-align: center;
  position: relative; /* Para posicionar o detalhe da cobra */
  overflow: hidden; /* Garante que o detalhe não vaze */
}

/* Detalhe da cobra coral com efeito de faixas */
